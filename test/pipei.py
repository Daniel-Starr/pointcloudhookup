...
def extract_towers(
    ...
    try:
        ...
        # 使用百分位数确定基准高度
        base_height = np.percentile(z_values, 25)  # 使用第25百分位作为基准

        # 动态计算高度阈值
        height_threshold = base_height + min_height * 0.7

        filtered_indices = z_values > height_threshold
        filtered_points = points[filtered_indices]

        log(f"✅ 高度过滤完成，基准高度: {base_height:.2f}m, 阈值: {height_threshold:.2f}m, 保留点数: {len(filtered_points)}")
    except Exception as e:
